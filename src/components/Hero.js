...
const Hero = () => {
  ...
  return (
    <div onMouseLeave={() => setIsHovered(false)} onMouseEnter={() => setIsHovered(true)} className="overflow-hidden">
     <Slider ref={sliderRef} {...settings}>
        {slides.map((slide, index) => (
          <div key={index}>
          <div className={`slider z-[777] relative h-[400px] lg:h-[818px] w-full ${index === 0 ? 'overlay' : ''}`} style={{background: `url(${slide.image})`, backgroundPosition: 'center', backgroundRepeat: "no-repeat", backgroundSize: 'cover'}}>
